...
# Konversi NAMA PRODI menjadi label numerik (untuk clustering)
label_encoder_prodi = LabelEncoder()
data['Cluster'] = label_encoder_prodi.fit_transform(data['NAMA PRODI'])
...
prodi_options = sorted(data['NAMA PRODI'].unique())  # Mengurutkan program studi dari A sampai Z
...
if st.button('Tampilkan Rekomendasi'):
    ...
    filtered_data = data[data['NAMA PRODI'] == selected_prodi]
    ...
    st.write(top_3[['PTN', 'MIN']])

    # 4. Membuat visualisasi scatter plot
    fig, ax = plt.subplots()
    scatter = ax.scatter(data['MIN'], data['Cluster'], c=data['Cluster'], cmap='viridis', label=data['PTN'])
